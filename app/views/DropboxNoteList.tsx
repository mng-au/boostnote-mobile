...
export interface IDropboxNoteListProps {
  isConnectedToDropbox: boolean;
  setIsConnectedToDropbox: (value: boolean) => void;
}
...
interface IDropboxNoteListState {
  ...
  isNoteOpen: boolean;
  isLoading: boolean;
  isNotConnectedToBoostnote: boolean;
  code: string;
}

export default class DropboxNoteList extends React.Component<
    ...
    fetch('https://content.dropboxapi.com/2/files/download', {
      ...
      .then((responseJson) => {
        ...
        if (
          ...
          responseJson.error_summary.startsWith('path/not_found/')
        ) {
          this.props.setIsConnectedToDropbox(true);
        }
        ...
        responseJson.entries.forEach((entry: any) => {
          ...
          fetch('https://content.dropboxapi.com/2/files/download', {
            ...
            .then((responseCson) => {
              ...
              const notesOfFolder = noteList.filter((folder) => {
  ...
  getAccessToken() {
    fetch(
      `https://api.dropboxapi.com/oauth2/token?code=${
        this.state.code
      }&grant_type=authorization_code&client_id=${settings.dropboxClientId}&client_secret=${
        settings.dropboxClientSecret
      }`,
      {
        method: 'POST',
        headers: {
      ...
      .then((response) => {
        AsyncStorage.setItem(DROPBOX_ACCESS_TOKEN, response.access_token)
          ...
          .catch((e) => {
            Alert.alert('Something wrong', 'Please retry', [{ text: 'OK' }], { cancelable: false });
          });
      })
      .catch((e) => {
        Alert.alert(
  ...
  render() {
    return (
      ...
      >
        ...
        !this.props.isConnectedToDropbox && !this.state.isLoading ? (
          <View>
            <View style={styles.dropboxLinkButtonWrap}>
              <Button
                style={styles.dropboxLinkButton}
                onPress={() =>
                  Linking.openURL(
        ...
        // 3. Not loading.
        this.props.isConnectedToDropbox &&
        this.state.isNotConnectedToBoostnote &&
                  ...
                  {folder &&
                    ...
                    folder.notes.map((note: INote) => {
                      return (
                        ...
                        >
                          <Body>
                            <Text numberOfLines={1} style={styles.noteListText}>
                              {note.content}
                            </Text>
                            <Text style={styles.noteListDate}>
                              {moment(note.updatedAt).format('MMM D')}
                            </Text>
                          </Body>
                        </CardItem>
                      );
