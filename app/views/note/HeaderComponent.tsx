...
import { Header, Left, Right, Button, Root, Icon } from 'native-base';
...
const HeaderLeft = ({ setIsOpen, folderName }) => (
  <Left style={Platform.OS === 'android' ? { top: 0 } : null}>
    <Button transparent onPress={() => setIsOpen('', false)}>
      <Text>
        <Icon name="ios-arrow-back" style={styles.noteDetailButton} />
      </Text>
  ...
  <Right style={Platform.OS === 'android' ? { top: 0 } : { top: 3 }}>
    <View>
      <Root>
        ...
        >
          <Text style={styles.switchEditText}>{isEditting ? 'Save' : 'Edit'}</Text>
        </Button>
        <Button transparent onPress={handlePressDetailButton}>
          <Text>
            <Icon name="ios-more" style={styles.noteDetailButton} />
          </Text>
        </Button>
      </Root>
    </View>
...
const HeaderComponent = ({
  ...
  >
    <HeaderLeft setIsOpen={setIsOpen} folderName={folderName} />
    <HeaderRight
      handleSwitchEditButtonClick={handleSwitchEditButtonClick}
